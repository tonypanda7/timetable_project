...
export async function datasetFileExists(filename) {
  const dir = filename.includes('/') ? filename.split('/').slice(0, -1).join('/') : '';
  const name = filename.includes('/') ? filename.split('/').slice(-1)[0] : filename;
  const list = await supabase.storage.from('datasets').list(dir, {});
  if (list.error) return false;
  return (list.data || []).some((f) => f.name === name);
}
...
export async function saveJSON(path, data) {
  ...
  const up = await supabase.storage.from('datasets').upload(path, payload, { contentType: 'application/json', upsert: true });
  if (up.error) throw new Error(up.error.message);
}

export async function loadJSON(path) {
  const dl = await supabase.storage.from('datasets').download(path);
  if (dl.error) throw new Error(dl.error.message);
  const text = await dl.data.text();
  return JSON.parse(text);
}

export async function saveTimetableAndIndexes(timetable) {
  await saveJSON('timetable.json', timetable);
  const teacherIndex = {};
  const classIndex = {};
  for (const e of timetable) {
    const [day, period] = String(e.slot).split('_');
    const simplified = { day, period: parseInt(period,10), course_id: e.course.id, course_name: e.course.course_name, room_id: e.room.id, room_name: e.room.location, group: e.group, students: e.students, teacher_id: e.teacher.id };
    if (!teacherIndex[simplified.teacher_id]) teacherIndex[simplified.teacher_id] = [];
    teacherIndex[simplified.teacher_id].push(simplified);
    const classKey = simplified.group || `${e.course.program_name || ''}_${e.course.semester || ''}`;
    if (!classIndex[classKey]) classIndex[classKey] = [];
    classIndex[classKey].push(simplified);
  }
  await saveJSON('indexes/teacher_index.json', teacherIndex);
  await saveJSON('indexes/class_index.json', classIndex);
}

export async function getTimetableFromStorage() {
  return loadJSON('timetable.json');
}
